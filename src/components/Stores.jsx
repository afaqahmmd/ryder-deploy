...
const Stores = ({ showConnectStoreModal, setShowConnectStoreModal }) => {
  ...
  const [storeForCoupons, setStoreForCoupons] = useState(null);
  ...
  const handleCoupons = (store) => {
    const coupons = [];
    store?.coupon_codes?.map((coupon) => {
      coupons.push(parsePythonDict(coupon));
    });

    setStoreForCoupons({ store, coupons });
  ...
  const handleAddCoupon = async (e) => {
    ...
    try {
      const couponToAdd = {
        ...
        expired_at: formatBackendDate(newCouponData.expired_at),
        percentage_off: newCouponData.percentage_off || null,
        fixed_amount_off: newCouponData.fixed_amount_off || null,
      };

      const updatedCoupons = [...storeForCoupons.coupons, couponToAdd];

      await axiosInstance.patch(`/api/stores/${storeForCoupons.store.id}/`, {
        coupon_codes: updatedCoupons,
      });
      ...
      // Refetch the store to get the latest coupon codes
      const response = await dispatch(
        fetchStoreById(storeForCoupons.store.id)
      ).unwrap();

      // Update the local state for the coupons modal
      handleCoupons(response);

      // Refresh the main stores list in the background
      dispatch(fetchStores());
  ...
  const handleDeleteCoupon = async (couponCode) => {
    ...
    setIsProcessingCoupon(true);
    try {
      const updatedCoupons = storeForCoupons.coupons.filter(
        (c) => c.code !== couponCode
      );
  ...
  return (
    <div className="p-6">
            ...
            {connectedStores.map((store, index) => (
              ...
              >
                ...
                <div className="flex space-x-2">
                  <button
                    onClick={() => handleCoupons(store)}
                    className="flex-1 bg-green-50 hover:bg-green-100 py-2 px-3 rounded-lg text-sm font-medium transition-colors"
                  >
      ...
      {/* Coupons Modal */}
      {showCouponsModal && storeForCoupons && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-4xl w-full overflow-hidden border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  Configure discount codes for{" "}
                  {storeForCoupons.store.store_name}
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <div className="p-6">
              {isAddingCoupon ? (
                <div className="bg-gray-50 dark:bg-gray-900/50 p-6 rounded-xl border border-gray-200 dark:border-gray-700 mb-6 animate-in slide-in-from-top-4 duration-300">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          type="date"
                          className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-green-500 outline-none transition-all"
                          value={newCouponData.expired_at}
                          onChange={(e) =>
                            setNewCouponData({
                              ...newCouponData,
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewCouponData({
                              ...newCouponData,
                              fixed_amount_off: e.target.value,
                            })
                          }
                        />
                      </div>
              ...
              <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                    {storeForCoupons.coupons.length === 0 ? (
                      <tr>
                        <td
                          colSpan="4"
                        ...
                        >
                          <div className="flex flex-col items-center">
                            <MdWarning className="w-12 h-12 text-gray-300 mb-2" />
                            <p>No coupons found for this store.</p>
                          </div>
                        </td>
                      </tr>
                    ) : (
                      storeForCoupons?.coupons?.map((coupon) => (
                        <tr
                          key={coupon?.code}
                          className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group"
                        >
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className="font-mono bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded text-green-600 dark:text-green-400 font-bold">
                              {coupon?.code}
                            </span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex flex-col">
                              {coupon?.percentage_off ? (
                                <span className="text-gray-900 dark:text-white font-medium">
                                  {coupon?.percentage_off}% Off
                                </span>
                              ) : null}
                              {coupon?.fixed_amount_off ? (
                                <span className="text-gray-900 dark:text-white font-medium">
                                  ${coupon?.fixed_amount_off} Off
                                </span>
                              ) : null}
                              {!coupon?.percentage_off &&
                                !coupon?.fixed_amount_off && (
            ...
            <div className="p-6 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700 flex justify-end">
              <button
                ...
                className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 px-6 py-2 rounded-lg font-bold hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors shadow-sm"
              >
                Close
              </button>
            </div>

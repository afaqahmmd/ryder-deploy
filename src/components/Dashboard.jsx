...
import {
  logoutUser as logoutSignupUser,
...
const Dashboard = () => {
  ...
  const [productDatabaseStatus, setProductDatabaseStatus] =
  ...
  const { agents, totalCount: agentsCount } = useSelector(
    (state) => state.agents
  );
  ...
  useEffect(() => {
    ...
    const signupOnboardingCompleted = localStorage.getItem(
      "signup_onboarding_completed"
    );
      ...
      try {
        ...
        if (!showOnboarding) {
          ...
          if (
            ...
            console.log(
              "🔍 DEBUG - Showing Regular Onboarding (no stores connected)"
            );
  ...
  const checkProductDatabaseStatus = async () => {
    try {
      ...
      const storeWithProducts = stores.find(
        (store) => productCounts[store.id] > 0
      );
                ...
                <div className="col-span-full bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 text-center">
                  <div className="mb-4">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-4">
                      Connect your first Shopify store to get started with AI
                      chatbots
                    </p>
